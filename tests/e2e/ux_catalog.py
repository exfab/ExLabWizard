...
UX_INTERACTIONS: tuple[UXInteraction, ...] = (
    ...
    # -- Settings: NAS credentials (rclone-only migration) -----------------
    UXInteraction(
        flow="NAS Credentials",
        route="/settings",
        testid="settings-nav-nas_credentials",
        element="nav row",
        action="Click the 'NAS Credentials' sidebar row",
        outcome="Shows one keyring-credential row per password-requiring equipment.",
    ),
    UXInteraction(
        flow="NAS Credentials",
        route="/settings",
        testid="settings-nas-password-EQ1",
        element="credential field",
        action="Set / Replace / Clear the equipment's NAS password",
        outcome="Writes the per-equipment password to the OS keyring at click time.",
    ),
    UXInteraction(
        flow="NAS Credentials",
        route="/settings",
        testid="settings-nas-test-EQ1",
        element="button",
        action="Click 'Test connection'",
        outcome="Runs the rclone probe and renders the result inline.",
    ),
    UXInteraction(
        flow="Equipment",
        route="/settings",
