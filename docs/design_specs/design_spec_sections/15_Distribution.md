...
6. **Code signing and macOS notarization (v1.1 commitment).** v1 ships unsigned per §15.2; v1.1 will sign. Decisions still required: cert provider (lab IT vs cloud-CA), Windows EV vs OV (EV bypasses SmartScreen reputation but costs more), macOS Apple Developer enrollment owner, CI integration (sign-on-tag vs sign-on-release), and how to handle the offline-machine first-launch when notarization stapling needs the OCSP / CRL call. Tracked here so the v1.1 commitment is concrete; defer until v1 ships.
